...
import { 
  ...
  CheckCircle, Smartphone, Award, Zap, ChevronRight, BarChart2, PieChart as PieChartIcon, Activity
} from "lucide-react";
...
const DetlicsSingleApp = () => {
  ...
  // Fetch related apps
  const { data: relatedAppsData } = useQuery({
    queryKey: ["relatedApps"],
    queryFn: async () => {
      const res = await apies.get("allapp");
      return res.data;
    }
  });
  ...
  // Filter related apps
  const relatedApps = relatedAppsData
    ? relatedAppsData.filter((app) => app.id !== id && app._id !== id).slice(0, 3)
    : [];
    ...
    <div className="bg-base-100 min-h-screen pb-20 overflow-x-hidden font-sans">
      ...
      <div className="relative bg-base-100 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 pt-8 pb-16">
          {/* Breadcrumb */}
          <Link to="/" className="inline-flex items-center gap-2 text-sm font-medium text-base-content/60 hover:text-primary transition-colors mb-12 group">
            ...
            <div className="flex-1 w-full pt-2">
              ...
              >
                ...
                <p className="text-xl text-base-content/60 mb-10 flex items-center gap-3 font-medium">
                  ...
                  Developed by <span className="text-base-content font-bold">{findApp.companyName}</span>
                </p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
                  <div className="bg-base-100/40 backdrop-blur-md p-5 rounded-3xl border border-white/20 shadow-lg hover:bg-base-100/60 transition-colors">
                    <div className="text-sm font-medium text-base-content/50 mb-1">Downloads</div>
                    <div className="text-2xl font-bold flex items-center gap-2 text-base-content">
                      <Download className="w-5 h-5 text-primary" /> {findApp.downloads ? (findApp.downloads > 1000000 ? (findApp.downloads/1000000).toFixed(1) + 'M' : findApp.downloads) : '10K+'}
                    </div>
                  </div>
                  <div className="bg-base-100/40 backdrop-blur-md p-5 rounded-3xl border border-white/20 shadow-lg hover:bg-base-100/60 transition-colors">
                    <div className="text-sm font-medium text-base-content/50 mb-1">Rating</div>
                    <div className="text-2xl font-bold flex items-center gap-2 text-base-content">
                      <Star className="w-5 h-5 text-warning fill-warning" /> {findApp.ratingAvg || '4.5'}
                    </div>
                  </div>
                  <div className="bg-base-100/40 backdrop-blur-md p-5 rounded-3xl border border-white/20 shadow-lg hover:bg-base-100/60 transition-colors">
                    <div className="text-sm font-medium text-base-content/50 mb-1">Size</div>
                    <div className="text-2xl font-bold text-base-content flex items-center gap-2">
                      <Smartphone className="w-5 h-5 text-secondary" /> {findApp.size || '25'} MB
                    </div>
                  </div>
                  <div className="bg-base-100/40 backdrop-blur-md p-5 rounded-3xl border border-white/20 shadow-lg hover:bg-base-100/60 transition-colors">
                    <div className="text-sm font-medium text-base-content/50 mb-1">Version</div>
                    <div className="text-2xl font-bold text-base-content flex items-center gap-2">
                       <Activity className="w-5 h-5 text-accent" /> {findApp.version || '1.0.2'}
                    </div>
                  </div>
                </div>
        ...
        <div className="lg:col-span-8 space-y-16">
          ...
          >
            ...
            <div className="prose prose-lg prose-headings:font-bold prose-p:text-base-content/80 max-w-none">
              ...
              {findApp.longDescription && <p className="leading-loose">{findApp.longDescription}</p>}
            </div>
          </motion.section>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(findApp.features || ['Real-time Synchronization', 'Advanced Analytics', 'Cloud Backup', 'Multi-user Support', 'Offline Mode', 'Dark Theme']).map((feature, idx) => (
                <motion.div 
                  whileHover={{ y: -5 }}
                  key={idx} 
                ...
                >
                  ...
                  <span className="font-semibold text-base-content/80">{feature}</span>
                </motion.div>
              ))}
            </div>
          </motion.section>
          ...
          >
            ...
            <div className="flex flex-col md:flex-row items-end justify-between mb-12 gap-6">
              <div>
                ...
                <p className="text-base-content/60 ml-16">Based on {findApp.reviews || '10,000+'} verified user reviews</p>
              </div>
              <div className="text-right">
                <div className="text-6xl font-black text-base-content tracking-tighter">{findApp.ratingAvg || '4.5'}</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="bg-base-200/30 rounded-3xl p-6 border border-base-content/5">
                <h3 className="font-bold text-lg mb-6 flex items-center gap-2">
                  <Activity size={18} className="text-primary" /> Rating Breakdown
                </h3>
                <div className="h-[250px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      />
                      <Bar dataKey="count" radius={[0, 10, 10, 0]} barSize={24} animationDuration={1500}>
              ...
              <div className="bg-base-200/30 rounded-3xl p-6 border border-base-content/5">
                <h3 className="font-bold text-lg mb-6 flex items-center gap-2">
                  <PieChartIcon size={18} className="text-secondary" /> Sentiment Distribution
                </h3>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none pb-8">
                     <div className="text-center">
                        <div className="text-2xl font-bold text-base-content">
                           {Math.round((chartData[4]?.count / chartData.reduce((a,b)=>a+b.count,0)) * 100) || 75}%
                        </div>
                        <div className="text-xs text-base-content/50 font-bold uppercase">5 Star</div>
                     </div>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-12 space-y-6">
              ...
              {[1, 2].map((i) => (
                <div key={i} className="p-6 rounded-2xl bg-base-100 border border-base-content/5 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-4 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs font-medium text-base-content/50">2 hours ago</div>
                    </div>
                    <div className="ml-auto flex gap-0.5">
                      {[1,2,3,4,5].map(star => <Star key={star} size={14} className="text-warning fill-warning" />)}
                    </div>
                  </div>
                  <p className="text-base-content/80 leading-relaxed italic">
                    "This app completely transformed how I manage my daily tasks. The interface is intuitive and the features are exactly what I needed. Highly recommended for anyone looking to boost productivity!"
                  </p>
                </div>
              ))}
              <button className="btn btn-outline btn-block rounded-xl">View All Reviews</button>
            </div>
          </motion.section>
        </div>
          ...
          <div className="bg-base-100 p-8 rounded-[2rem] border border-base-content/10 shadow-2xl sticky top-24">
            ...
            <ul className="space-y-6">
              <li className="flex justify-between items-center pb-4 border-b border-base-content/5">
                <span className="text-base-content/60 font-medium">License</span>
                <span className="font-bold text-sm bg-base-200 px-4 py-1.5 rounded-full">{findApp.license || 'Free'}</span>
              </li>
              <li className="flex justify-between items-center pb-4 border-b border-base-content/5">
                <span className="text-base-content/60 font-medium">Category</span>
                <span className="font-bold text-sm text-right">{findApp.category || 'Productivity'}</span>
              </li>
              <li className="flex justify-between items-center pb-4 border-b border-base-content/5">
                <span className="text-base-content/60 font-medium">Languages</span>
                <span className="font-bold text-sm text-right">{findApp.languages || 'English, Spanish'}</span>
              </li>
              <li className="flex justify-between items-center pb-4 border-b border-base-content/5">
                <span className="text-base-content/60 font-medium">Age Rating</span>
                <span className="font-bold text-sm bg-base-200 px-3 py-1 rounded-lg">{findApp.ageRating || '4+'}</span>
              </li>
              <li className="flex justify-between items-center pb-4 border-b border-base-content/5">
                <span className="text-base-content/60 font-medium">Last Updated</span>
                <span className="font-bold text-sm flex items-center gap-2">
                  <Calendar size={14} /> {findApp.updatedAt ? new Date(findApp.updatedAt).toLocaleDateString() : 'Oct 20, 2025'}
                </span>
              </li>
            </ul>
            ...
            {relatedApps.length > 0 && (
              <>
                <div className="divider my-8 font-bold text-base-content/30">Similar Apps</div>
                
                <div className="space-y-5">
                  {relatedApps.map((app) => (
                    <Link 
                      to={`/appDetlics/${app.id || app._id}`} 
                      key={app.id || app._id} 
                      className="group flex gap-4 items-center p-3 rounded-2xl hover:bg-base-200/50 transition-all border border-transparent hover:border-base-content/5"
                    >
                      <img 
                        src={app.image} 
                        alt={app.title} 
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-base truncate group-hover:text-primary transition-colors">{app.title}</h4>
                        <div className="flex items-center gap-2 text-xs text-base-content/60 mt-1">
                          <span className="flex items-center gap-1 bg-warning/10 px-1.5 py-0.5 rounded text-warning font-bold">
                             <Star className="w-3 h-3 fill-current" /> {app.ratingAvg || '4.5'}
                          </span>

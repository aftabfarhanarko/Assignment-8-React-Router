...
import { 
  ...
  LayoutDashboard, ShoppingBag, Settings, LogOut, ChevronRight,
  MoreHorizontal, Filter, Download, CreditCard, Wallet, Calendar
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-base-200 flex font-sans overflow-hidden relative selection:bg-primary/20">
      ...
      `}>
        ...
        <nav className="flex-1 px-6 space-y-2 overflow-y-auto py-4 scrollbar-hide">
          <p className="px-4 text-xs font-bold text-base-content/40 uppercase tracking-wider mb-4">Main Menu</p>
          {[
            ...
            >
              ...
              {!item.active && <ChevronRight size={16} className="ml-auto opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all" />}
            </a>
          ))}

          <p className="px-4 text-xs font-bold text-base-content/40 uppercase tracking-wider mt-8 mb-4">General</p>
          <a href="#" className="flex items-center gap-4 px-4 py-3.5 text-base-content/60 hover:bg-base-200 hover:text-base-content rounded-2xl font-medium transition-all duration-300 group">
            <Settings size={20} className="group-hover:rotate-90 transition-transform duration-500" /> Settings
          </a>
        </nav>
        ...
        <div className="p-6 border-t border-base-content/5 relative">
          <div className="relative bg-gradient-to-tr from-primary/10 to-secondary/10 p-5 rounded-2xl border border-primary/10 mb-4 overflow-hidden group">
            ...
            <p className="text-xs text-base-content/60 mb-3 relative z-10">Get 20% off on your next upgrade.</p>
            <button className="btn btn-xs btn-primary w-full relative z-10 shadow-lg shadow-primary/20 border-none">Upgrade Now</button>
          </div>
      ...
      <main className="flex-1 overflow-x-hidden relative z-10 h-screen overflow-y-auto scroll-smooth">
        ...
        <header className="bg-base-100/70 backdrop-blur-xl border-b border-white/10 p-4 lg:p-6 flex justify-between items-center sticky top-0 z-30 transition-all duration-300 supports-[backdrop-filter]:bg-base-100/50">
          <div className="flex items-center gap-4">
            ...
            <div className={`relative hidden md:block group transition-all duration-300 ${searchFocused ? 'w-96' : 'w-80'}`}>
              <Search className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${searchFocused ? 'text-primary' : 'text-base-content/40'}`} />
              <input 
                type="text" 
              ...
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex gap-1">
                <kbd className="kbd kbd-sm text-[10px] font-sans bg-base-100/50">⌘</kbd>
                <kbd className="kbd kbd-sm text-[10px] font-sans bg-base-100/50">K</kbd>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
            <div>
              ...
              <p className="text-base-content/60 font-medium">Welcome back! Here's your overview.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { label: 'Total Revenue', value: '$54,239', icon: DollarSign, color: 'text-success', bg: 'bg-success', trend: '+12.5%' },
              { label: 'Total Users', value: '2,543', icon: Users, color: 'text-primary', bg: 'bg-primary', trend: '+8.2%' },
              { label: 'New Sales', value: '345', icon: ShoppingBag, color: 'text-warning', bg: 'bg-warning', trend: '-2.4%' },
              { label: 'Bounce Rate', value: '42.3%', icon: Activity, color: 'text-error', bg: 'bg-error', trend: '+4.1%' },
            ].map((stat, index) => (
              <motion.div 
                key={index} 
              ...
              >
                <div className={`absolute -top-10 -right-10 w-40 h-40 ${stat.bg} opacity-[0.05] rounded-full group-hover:scale-150 transition-transform duration-700 ease-in-out blur-3xl`} />
                <div className="flex justify-between items-start mb-6 relative z-10">
                  <div className={`p-3.5 rounded-2xl ${stat.bg}/10 ${stat.color} shadow-sm ring-1 ring-inset ring-white/20`}>
                    <stat.icon size={26} />
                  </div>
                  <span className={`flex items-center gap-1 text-xs font-bold px-2.5 py-1.5 rounded-lg ${stat.trend.startsWith('+') ? 'bg-success/10 text-success' : 'bg-error/10 text-error'} border border-transparent group-hover:border-current/10 transition-colors`}>
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 relative z-10 gap-4">
                <div>
                  ...
                  <p className="text-sm text-base-content/50 ml-4">Income vs Expenses over time</p>
                </div>
                <div className="flex gap-2 bg-base-200/50 p-1 rounded-xl backdrop-blur-md">
                  {['Week', 'Month', 'Year'].map((t) => (
                    <button key={t} className={`btn btn-sm rounded-lg border-none ${t === 'Year' ? 'bg-white shadow-sm text-base-content' : 'btn-ghost text-base-content/60'}`}>{t}</button>
                  ))}
                </div>
              </div>
              <div className="h-[350px] w-full relative z-10">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={revenueData}>
                    ...
                    <Tooltip 
                      contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(10px)', border: 'none', borderRadius: '16px', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1)', color: '#1f2937' }}
                      itemStyle={{ color: '#1f2937', fontWeight: 600 }}
                      cursor={{ stroke: '#6366f1', strokeWidth: 1, strokeDasharray: '5 5' }}
                    />
                    <Legend iconType="circle" wrapperStyle={{ paddingTop: '20px' }} />
                    <Area type="monotone" dataKey="revenue" stroke="#6366f1" fillOpacity={1} fill="url(#colorRevenue)" strokeWidth={4} />
                    <Area type="monotone" dataKey="cost" stroke="#10b981" fillOpacity={1} fill="url(#colorCost)" strokeWidth={4} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="flex justify-between items-start mb-6 relative z-10">
                <div>
                  ...
                  <p className="text-sm text-base-content/50 ml-4">Sessions by device type</p>
                </div>
                <button className="btn btn-circle btn-ghost btn-sm"><MoreHorizontal size={20} /></button>
              </div>
              ...
              <div className="flex-1 min-h-[300px] relative z-10">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip 
                       contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(8px)', border: 'none', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
                       itemStyle={{ color: '#1f2937', fontWeight: 600 }}
                    />
                  </PieChart>
                </ResponsiveContainer>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-4xl font-black text-base-content">85%</span>
                  <span className="text-sm text-base-content/50 font-medium">Mobile</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 mt-6 relative z-10">
                 {deviceData.map((item, idx) => (
                   <div key={idx} className="flex items-center gap-2 bg-white/30 p-2 rounded-xl">
                     <div className="w-3 h-3 rounded-full shadow-sm" style={{backgroundColor: item.color}}></div>
                     <span className="text-sm font-medium text-base-content/70">{item.name}</span>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={userGrowthData} barGap={8}>
                    ...
                    <Tooltip 
                      ...
                      contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(8px)', border: 'none', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
                    />
                    <Bar dataKey="users" fill="#6366f1" radius={[6, 6, 6, 6]} barSize={16}>
                      {userGrowthData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fillOpacity={0.8 + (index * 0.05)} />
                      ))}
                    </Bar>
                    <Bar dataKey="active" fill="#e5e7eb" radius={[6, 6, 6, 6]} barSize={16} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </motion.div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={revenueData}>
                    ...
                    <Tooltip 
                       contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(8px)', border: 'none', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
                    />
                    <Line type="monotone" dataKey="revenue" stroke="#f59e0b" strokeWidth={4} dot={{r: 0}} activeDot={{r: 8, strokeWidth: 0, fill: '#f59e0b', stroke: '#fff', strokeWidth: 3}} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </motion.div>
          ...
          >
            <div className="flex justify-between items-center mb-8">
              ...
              <button className="btn btn-sm btn-ghost text-primary hover:bg-primary/10 rounded-lg">View All</button>
            </div>
              ...
              <table className="table w-full border-separate border-spacing-y-3">
                <thead>
                  <tr className="text-base-content/40 font-semibold uppercase tracking-wider text-xs">
                    ...
                    <th className="py-4 bg-transparent border-none pr-6">Status</th>
                  </tr>
                </thead>
                <tbody className="text-sm">
                  {[1, 2, 3, 4, 5].map((item) => (
                    ...
                    >
                      <td className="font-mono text-xs font-medium opacity-60 py-5 pl-6 rounded-l-2xl border-none">#TRX-78{item}9</td>
                      <td className="py-5 border-none">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="font-bold text-sm text-base-content/90">User Name {item}</div>
                            <div className="text-xs opacity-50">user{item}@example.com</div>
                          </div>
                        </div>
                      </td>
                      <td className="font-medium text-sm py-5 border-none">Premium Plan</td>
                      <td className="text-sm opacity-70 py-5 border-none">Oct 24, 2025</td>
                      <td className="font-bold text-sm py-5 border-none">$99.00</td>
                      <td className="py-5 pr-6 rounded-r-2xl border-none">
                        <span className={`badge border-0 font-bold py-3 px-4 ${item % 2 === 0 ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'} group-hover:scale-105 transition-transform`}>
